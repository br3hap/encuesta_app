...
class Survey(main.Survey):
    ...
    @http.route(['/web/binary/preview/<int:file_id>'], type='http',
                auth='user', website=True, sitemap=False)
    def binary_preview(self, file_id=None, **post):
        if file_id:
            file = request.env['ir.attachment'].sudo().browse(file_id)
            if file and file.datas:
                content = base64.b64decode(file.datas)
